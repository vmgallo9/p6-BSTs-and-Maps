...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    ...
    //RECURSIVE CASE
    //size of tree is root + size of left subtree + size of right 
  ...
  static Node *copy_nodes_impl(Node *node) {
    //BASE CASE - Empty tree returns an empty root node (nullptr?) 
     if (!node) {
      return nullptr;  
    }

    //RECURSIVE CASE
    //creating new_node with same datum as node
    //and setting left and right to be copies of left and right subtrees
    Node* new_node = new Node{node->datum, copy_nodes_impl(node->left), 
  ...
  static void destroy_nodes_impl(Node *node) {
    //BASE CASE - Empty tree is already an empty node
     if (!node) {
      return; 
    }

    //RECURSIVE CASE
    destroy_nodes_impl(node->left); //delete memory for left subtree
    destroy_nodes_impl(node->right); //delete memory for right 
    delete node; //delete root node after deleting subtrees
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    //BASE CASE - Empty tree or elt is not found -> return a null ptr 
    if (!node) {
      return nullptr; 
    }
    ...
    //if node datum is greater than query 
    //go left
    else if (less(query, node->datum)) {
      return find_impl(node->left, query, less); 
    }
    //if node datum is less than query 
    //go right
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    if (!node) {
      //left and right subtrees with be nullptrs
